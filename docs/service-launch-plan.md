...
The portfolio should route reviewers into four scoped technical lanes:

| Service lane | First reviewer | First motion | Architecture anchor |
| --- | --- | --- | --- |
...
| --- | --- | --- |
| Custom domain | Makes review and sharing cleaner than a GitHub subpath. | After the public brand/domain name is chosen. |
...
| 3-5 | Build a target-review list across the four service lanes. | Each account has one specific workflow pain. |
...
| 11-14 | Run calls and pitch the smallest reviewed workflow, not a vague custom build. | One reviewer gives enough scope to justify backend resources. |

## Guardrails

- Present diagnostics first when data access, security review, or reviewer authority is unclear.
...
- Do not build more features until a reviewer confirms the workflow.
